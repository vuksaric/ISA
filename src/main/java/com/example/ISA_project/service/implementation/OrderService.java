    ...
    public void deleteOrder(int id) {
        try{
            ...
            if(orderList.getOffers().size() == 0){
                orderList.setStatus(OrderStatus.Rejected);
                orderRepository.save(orderList);
            }
        }catch (Exception e){
            e.printStackTrace();
    ...
    public List<MedicineOrderDTO> getAll(int id) {
        ...
        try{
            for(OrderList ol : orderRepository.findAll()){
                if(ol.getPharmacyId() == id && ol.getStatus() == OrderStatus.Waiting){
        ...
        try{
            ...
            for(Offer o : orderList.getOffers()){
                if(o.getId() == acceptOfferDTO.getOfferId()){
                    ProfileDTO profileDTO = userService.getProfile(o.getSupplierId());
            ...
            orderList.setStatus(OrderStatus.Accepted);
            orderRepository.save(orderList);
